...
describe("GameDetailModal", () => {
  it("shows only this game's player props after fetch", async () => {
    ...
    const api: SportApi = { games: vi.fn(), gamePrediction: vi.fn().mockResolvedValue(prediction), playerProps: vi.fn().mockResolvedValue([prop("home-player","Ravens"), prop("other","Bengals")]), trackRecord: vi.fn(), retrain: vi.fn() };
